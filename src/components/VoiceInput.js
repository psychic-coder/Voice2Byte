...
export default function VoiceInput() {
  ...
  const processAudio = async (audioBlob) => {
    ...
    try {
      ...
      const context = {
        currentPage: pathname.replace("/", "") || "home",
        restaurants: restaurants.map((r) => r.name),
        menuItems: currentRestaurantRef.current?.menu?.map((m) => m.item) || [],
        cart: cartItemsRef.current || [],
        userLocation: userLocationRef.current || "",
        totalPrice: totalPriceRef.current || 0,
      };
  ...
  const handleIntent = (decision) => {
    ...
    switch (action) {
      case "sayPage":
        if (decision.page) SpeakText(`You are on the home page.`);
        break;
      ...
      case "addToCart":
        decision.items?.forEach((item) => {
          addItemToCart(item.name, item.quantity);
          SpeakText(`Added ${item.quantity} ${item.name} to your cart.`);
        });
        break;

      case "readCart":
        if (decision.cart?.length) {
          const items = decision.cart
            .map((i) => `${i.quantity} ${i.name}`)
            .join(", ");
          SpeakText(`Your cart has: ${items}. Total: ${decision.totalPrice}`);
        } else SpeakText("Your cart is empty.");
        break;

      case "removeFromCart":
        if (decision.item) {
          removeItemFromCart(decision.item);
          SpeakText(`Removed ${decision.item} from cart.`);
        }
        break;

      case "checkout":
        router.push("/checkout");
        SpeakText(`Proceeding to checkout. Total: ${decision.totalPrice}`);
        break;
