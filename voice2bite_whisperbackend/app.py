...
resilience_event_writer = ThreadPoolExecutor(max_workers=1, thread_name_prefix="resilience-events")
...
breaker_storage = pybreaker.CircuitRedisStorage(
    pybreaker.STATE_CLOSED,
    redis_client,
    namespace="voice2bite:openrouter",
)


def persist_resilience_event(old_state, new_state):
    ...
    try:
        ...
        print(f"[Resilience] Logged breaker transition {old_state.name} -> {new_state.name} in {elapsed_ms:.2f}ms")
    except Exception as e:
        print(f"Failed to log state change to DB asynchronously: {e}")

class DBListener(pybreaker.CircuitBreakerListener):
    def state_change(self, cb, old_state, new_state):
        try:
            start_time = time.perf_counter()
            resilience_event_writer.submit(persist_resilience_event, old_state, new_state)
            elapsed_ms = (time.perf_counter() - start_time) * 1000
            print(f"[Resilience] Scheduled breaker transition logging in {elapsed_ms:.2f}ms")
        except Exception as e:
            print(f"Failed to schedule state change logging: {e}")

openrouter_breaker = pybreaker.CircuitBreaker(
    fail_max=3,
    reset_timeout=30,
    listeners=[DBListener()],
    state_storage=breaker_storage,
